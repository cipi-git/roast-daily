...
export default function ProcessAnalyzer({ lang = "it" }) {
  ...
  const labels = {
    ...
    summary: "Sintesi",
    opportunities: "Opportunità di automazione",
    ...
    <section className="mx-auto max-w-5xl py-10">
      <div className="rounded-3xl bg-zinc-950 p-6 text-white shadow-xl ring-1 ring-zinc-800 sm:p-8">
        ...
        <textarea
          ...
          className="mt-5 w-full rounded-2xl bg-white p-4 text-sm text-zinc-900 outline-none ring-1 ring-white/20 transition focus:ring-2 focus:ring-white"
        />
        ...
        <button
          ...
          className="mt-4 rounded-full bg-white px-5 py-2.5 text-sm font-bold text-zinc-950 shadow-md transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        {result && (
          <div className="mt-6 space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <List title={labels.opportunities} items={result.opportunities} />
              <List title={labels.workflow} items={result.workflow} />
              <List title={labels.risks} items={result.risks} />
              <List title={labels.next} items={result.next_steps} />
            </div>
